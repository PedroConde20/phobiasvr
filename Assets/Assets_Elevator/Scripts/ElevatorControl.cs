...
public class ElevatorControl : MonoBehaviour
{
   ...
   public Transform    m_weightPulley;
	...
	void Update ()
    {
        switch ( currentState )
        {
            ...
            case ELEVATOR_STATE.HEADING_TO_FLOOR:
                StateHeadingToFloor();
                // Llama al método para mover el objeto con el tag "moverse"
                UpdateObjectsWithMoveTag();
                break;
        }
    ...
    void UpdateObjectsWithMoveTag()
    {
        if (targetFloors.Count > 0)
        {
            int goalFloor = targetFloors[0];

            // Calcula la posición Y del objeto en función del piso actual del ascensor menos uno
            float newYPosition = goalFloor - 1;

            // Encuentra el objeto con el tag "moverse"
            GameObject[] objectsToMove = GameObject.FindGameObjectsWithTag("moverse");
